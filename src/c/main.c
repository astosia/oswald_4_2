...
static void bluetooth_vibe_icon (bool connected) {
 ...
 /* if(!connected && !quiet_time_is_active()) {
  ...
  if(!connected && !IsQuietNow){
    vibes_double_pulse();
    APP_LOG(APP_LOG_LEVEL_DEBUG, "BT vibes on");
  }
...
static void quiet_time_icon () {
 if (!IsQuietNow) {
   ...
   APP_LOG(APP_LOG_LEVEL_DEBUG, "QTIcon is hidden");
 }
 else {
     layer_set_hidden(s_canvas_qt_icon,false);
   APP_LOG(APP_LOG_LEVEL_DEBUG, "QTIcon is visible");
  }
...
static void onreconnection(bool before, bool now){
  if (!before && now){
    APP_LOG(APP_LOG_LEVEL_DEBUG, "BT reconnected, requesting quiet time status");
    request_watchjs();
  }
}
...
static void prv_inbox_received_handler(DictionaryIterator * iter, void * context){
 ...
 ///////////////////////////////
  ...
  if (tx6_color_t){
   ...
   // Get quiet time handlers
  Tuple * startquiet = dict_find(iter, MESSAGE_KEY_QuietStart);
  if (startquiet){
    settings.QuietStart = (int) startquiet -> value -> int32;
    APP_LOG(APP_LOG_LEVEL_DEBUG, "QTOn at %d", settings.QuietStart );
  }
  Tuple * endquiet = dict_find(iter, MESSAGE_KEY_QuietEnd);
    if (endquiet){
    settings.QuietEnd = (int) endquiet -> value -> int32;
    APP_LOG(APP_LOG_LEVEL_DEBUG, "QTOff at %d", settings.QuietEnd);
  }
   
  
  Tuple * disqtime_t = dict_find(iter, MESSAGE_KEY_QuietTime);
  if (disqtime_t){
    if (disqtime_t -> value -> int32 == 0){
      settings.QuietTime = false;
      APP_LOG(APP_LOG_LEVEL_DEBUG, "QTime off");
    } else settings.QuietTime = true;
  } 
...
/*  Tuple * disntheme_t = dict_find(iter, MESSAGE_KEY_NightTheme);
  ...
  //Update colors
  //quiet_time_icon();  //last thinh added
  ...
  // quiet_time_icon(); //last thing added
...
static void tick_handler(struct tm * time_now, TimeUnits changed){
 ...
 //0 = leave vibes on all the time
 IsQuietNow = false;
   APP_LOG(APP_LOG_LEVEL_DEBUG, "1st step, toggle is off, so false");
 }
  //1 = 
  else if (
    //If start time = end time,
    settings.QuietStart  == settings.QuietEnd ){
      //True = turn vibes off all the time,
      IsQuietNow = true;
      APP_LOG(APP_LOG_LEVEL_DEBUG, "2nd step met, same start and end so true");
  }
      //False =
  else if (
        //If end time < start time (blue)
        settings.QuietEnd  < settings.QuietStart) {
          //True =
            // If now <= start time && now >= end time,
              if (nowthehouris <= settings.QuietStart *100 && nowthehouris >= settings.QuietEnd *100 ) {
                   //True = vibes on
                   IsQuietNow = false;
                   APP_LOG(APP_LOG_LEVEL_DEBUG, "3rd step met, false");}
                   //False = vibes off
                   else {IsQuietNow = true;
                   APP_LOG(APP_LOG_LEVEL_DEBUG, "4th step met, true");}
          //False (ie when end time > start
          //           time(red)) =
  }
  else if(
         settings.QuietEnd > settings.QuietStart) {
          ...
          if (nowthehouris >= settings.QuietStart *100  && nowthehouris <= settings.QuietEnd *100) {    
                    //True = vibes off
                    IsQuietNow = true;
                    APP_LOG(APP_LOG_LEVEL_DEBUG, "5th step met, true");}
                    //False = vibes on
                    else {IsQuietNow = false;
                           APP_LOG(APP_LOG_LEVEL_DEBUG, "6th step met, false");}
  }
  //quiet_time_icon ();
